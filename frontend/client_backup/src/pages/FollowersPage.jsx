...
export default function FollowersPage() {
  ...
  const loadPage = async (p) => {
    ...
    try {
      const res = await API.get(`/users/${id}/followers?page=${p}&limit=${limit}`);
      ...
      if (err.response && (err.response.status === 403 || (err.response.data && err.response.data.private))) {
        ...
        console.error('Failed to load followers page', err);
      }
    } finally {
      setLoading(false);
    }
      ...
      {privateList ? (
        ...
        <div className="space-y-2">
          ...
          {items.length === 0 && !loading && (
            <div className="text-center text-gray-500 p-6">No followers yet</div>
          )}
        </div>
      )}
    </div>
